...
| `/harness:smell-scan` | Build code-review step (utility sub-scan): invoked by code-reviewer on changed source files — advisory Fowler-catalog smell sweep (Feature Envy, Data Clumps, Primitive Obsession, Message Chains, Shotgun Surgery, Divergent Change, Middle Man, Inappropriate Intimacy, Speculative Generality); ranked P1–P3 candidates with tag column; findings fold into code-reviewer output as advisory section; never a gate. | SMELLS_FOUND / SMELLS_CLEAN |
| `/harness:debt-ledger` | User-invocable utility: advisory grep-collector that harvests every `DEBT:` deliberate-simplification marker across the tree (excluding node_modules/.git/dist/build/target/.claude/worktrees), renders a ledger grouped by file, and flags `no-trigger` entries (no upgrade-trigger = silent rot); never a gate. Run periodically or at Reflect to track accumulating debt. | DEBT_LEDGER_WRITTEN / DEBT_LEDGER_CLEAN |
