...
- `metrics/{session}/phase-boundary.jsonl` — tokens-into-next-phase delta per phase boundary. Fields: `{ts, phase_from, phase_to, tokens_before, tokens_after, goal_retained, last_n_full, mode}`. `tokens_before` = token count of the raw accumulated phase notes; `tokens_after` = token count after goal+ACs verbatim + last-N-full + summary of the rest. Token unit: `ceil(utf8_bytes / 3.5)` — same constant both sides so the ratio is robust. Written by `hooks/phase-boundary-compress.sh` (advisory mode, default). **Enforcement-flip criterion (R5)**: flip from advisory to enforce when `metrics/*/phase-boundary.jsonl` across ≥10 boundaries shows median `tokens_after/tokens_before ≤ 0.7` AND zero phase-quality regressions in the interim (no extra review rounds attributable to lost handoff detail). The flip itself is a one-line default change; revert = remove the step.
