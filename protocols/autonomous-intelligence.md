...
**Schema-gap evidence (probe RED, 2026-05-14)**: the `promote-advisory-hooks-enforcement` pipeline (2026-05-14) probed whether `modified_tool_input` round-trips on the Agent matcher at v2.1.141. Verdict: RED — the probe could not be safely executed from within a build subagent (settings.json mutation would expose the parent session to cross-hook breakage), and documentary evidence in `protocols/thinking-defaults.md:101-110` confirms the per-spawn schema is still missing. See `pipeline-state/promote-advisory-hooks-enforcement/probe-result.md` for the full verdict, environment capture, and re-probe protocol for a future operator-driven session. Until `modified_tool_input` lands AND is verified by manual probe, all mutation-semantic flips (`pre-agent-thinking.sh`, `pre-agent-advisor.sh`, `cache-breakpoint-injector.sh`, this hook) remain advisory.
