...
> "Before completing, write any discoveries to the pipeline scratchpad at `$state_dir/{task-id}/scratchpad/{your-role}-{phase}.md`. Format below. Only write genuinely useful findings — not task narration."
...
4. **Compaction survival**: The notes file persists on disk. After context compaction, `/harness:pipeline-resume` re-reads it automatically. This is the primary mechanism for maintaining coherence across long runs.
...
**30-day DUAL_PATH soak.** Writers emit only the new layout; readers tolerate both forms via `session_memory_read_split` in `hooks/_lib/session-memory-read-split.sh`. The helper returns the new-layout file when present, falling back to the matching canonical section of the legacy single-file otherwise; each fallback hit appends a forensic JSONL line at `metrics/{session-id}/session-store-mirror.jsonl` with `source: "session-memory-read-fallback"`. The soak ends 30 days after merge, gated by the placeholder `$state_dir/wave2a-c3-soak-end/pipeline.md` (frontmatter `not_before` carries the calendar anchor; SessionStart's active-pipeline scan surfaces it once the date passes). The follow-up cleanup pipeline removes the reader-fallback code path and any remaining legacy artifacts.
...
**Soak end (4-item cleanup).** The placeholder pipeline at `$state_dir/auto-codebase-map-soak-end/pipeline.md` carries the `not_before:` calendar anchor (merge_date + 30 days). When the date passes, the cleanup pipeline executes exactly **4 cleanup items**:
...
# Append to learning/{project-hash}/observations.jsonl
{
  ...
  "triggered_by_pipeline_id": "<prior-task-id>" // OPTIONAL — omit when unknown; see Field reference
}
```

After the deploy phase completes (post-merge), a companion `deploy_outcome` record is appended by `/harness:deploy` or `/harness:deployment-verification` — it is a SEPARATE append, keyed by `pipeline_id`, because the deployment outcome is unknown at the time the `pipeline` record is written (pre-Ship):

```bash
# Companion append — post-merge, keyed by pipeline_id
{
...
`outcome` enum: `DEPLOYED` (deploy succeeded, verification passed), `ROLLED_BACK` (manual rollback after deploy), `AUTO_ROLLBACK` (deployment-verification triggered automatic rollback), `DEPLOY_FAILED` (deploy attempted but never reached production). Written post-merge by `/harness:deploy` (DEPLOYED / DEPLOY_FAILED / ROLLED_BACK) and `/harness:deployment-verification` (AUTO_ROLLBACK). The two records are joined at read time by `pipeline_id` — there is at most one `deploy_outcome` per `pipeline_id`; if both deploy and verification write one, the MAX-timestamp record wins (last-writer-wins: AUTO_ROLLBACK supersedes an earlier DEPLOYED).
...
| `phases.sandbox_verify` | object `{verdict, rounds, cost_estimate_usd, mode?, divergence_count?, skip_reason?, diverging_tests?}`, OR absent | written by Reflect step from the Build-phase Step 5b `/harness:sandbox-verify` gate (added in the sandbox-verify epic, Stories 1-4). `verdict` is one of `SANDBOX_VERIFIED`, `SANDBOX_FAILED`, `SANDBOX_SKIPPED`. `rounds` is the count of sandbox-verify cycles in this pipeline (initial + any re-runs after fix-engineer rework; shares the combined 2-round budget with code-review per `skills/build-implementation/SKILL.md` Step 5b). `cost_estimate_usd` is the sandbox spend captured from `metrics/{session-id}/sandbox-verify-cost.jsonl` teardown events (may be `0.0` or absent when cost data is unavailable). `mode` is OPTIONAL with values like `"diff-only" \| "exec"` reserved for forward-compatibility with future verifier modes. `divergence_count` is present iff `verdict == SANDBOX_FAILED` — the count of test rows where worktree and sandbox disagreed. `skip_reason` is present iff `verdict == SANDBOX_SKIPPED`, with enum values `"no-e2b-token" \| "no-testable-changes" \| "env-hatch" \| "e2b-unavailable" \| "cost-exceeded"`. `diverging_tests` is OPTIONAL and present iff `verdict == SANDBOX_FAILED` — a `list[str]` of test names from `$state_dir/{task-id}/build.md` § Sandbox Verify (bounded at 20 entries to keep observation lines under any downstream size cap; truncation is logged in the build agent's scratchpad when applied). Readers (`/harness:learn`, `/harness:forensics`, `/harness:eval-model-effectiveness`, `/harness:cost-report`) MUST tolerate absence per existing schema-compatibility rules — absence means the pipeline did not run a `/harness:sandbox-verify` Build-phase gate (legacy pipeline or build-only docs change), NOT a synthetic `SANDBOX_VERIFIED` + 0 cost. Mining via `learn_sandbox_fragility_mining.mine_sandbox_fragility` (`/harness:learn` Step 3) recurrence-3-gates on `diverging_tests` to emit `fragility` instincts at confidence 0.5 with roles `[software-engineer, sandbox-verify-engineer]`. |
| `triggered_by_pipeline_id` | string (a prior pipeline's task-id), OR absent | written by the Step 4d-i producer ONLY when the fix/bug pipeline's regression-originating pipeline is known (user-provided at intake via Discussion Persistence § Impact on Implementation, or engineer-identified at bug-fix Step 0). Root-level field on the `pipeline` record, sister to `classification` — NOT nested under `phases`. Absence = not a tracked post-merge regression of a known prior pipeline, NEVER coerced to a synthetic/guessed link. |
| `record_type: "deploy_outcome"` | companion record appended post-merge to the same `learning/{project-hash}/observations.jsonl` file | written by `/harness:deploy` (DEPLOYED / DEPLOY_FAILED / ROLLED_BACK) and `/harness:deployment-verification` (AUTO_ROLLBACK) after the deploy phase completes. Keyed by `pipeline_id` to join with the pre-Ship `pipeline` record. At most one `deploy_outcome` per `pipeline_id`; last-writer-wins by timestamp. This is an **additive telemetry record only** — no pipeline gate reads or acts on it. |
| `outcome` | string enum `{DEPLOYED, ROLLED_BACK, AUTO_ROLLBACK, DEPLOY_FAILED}`, OR absent (record not written) | field on the `deploy_outcome` record. `DEPLOYED` = deploy succeeded and verification passed; `ROLLED_BACK` = manual rollback triggered after deploy; `AUTO_ROLLBACK` = `/harness:deployment-verification` detected failure and triggered automatic rollback; `DEPLOY_FAILED` = deploy attempted but never reached production. Readers MUST tolerate absence of the `deploy_outcome` record entirely — absence means "not-yet-observed", NEVER coerced to a synthetic clean/success. |
...
**Backward compatibility:** readers (`/harness:learn`, `/harness:forensics`, `/harness:eval-model-effectiveness`) MUST tolerate absence of `cost_estimate_usd` in legacy records written before this field was introduced. Treat absence as "unknown cost", NOT as `0.0` — a missing field is not the same as a free pipeline. Aggregations that need cost data should filter out records where the field is missing rather than coercing them to zero (which would skew downward averages).
...
**Backward compatibility — `deploy_outcome` record:** readers (`/harness:learn` Step 7c-bis) MUST tolerate absence of the `deploy_outcome` record for any `pipeline_id`. Absence means "not-yet-observed" — the pipeline may not have deployed, or may predate this telemetry. Treat absence as 'not-yet-observed', NEVER coerced to a synthetic clean/success — a missing record is not the same as a successful deployment. Aggregations MUST filter out pipelines where the `deploy_outcome` record is missing rather than coercing them to a successful or failed outcome. The `escape_rate` denominator counts only pipelines that have a `deploy_outcome` record; pipelines without one are excluded from both numerator and denominator.

**Backward compatibility — `triggered_by_pipeline_id` field:** readers (`/harness:learn` Step 7c-ter) MUST tolerate absence of the `triggered_by_pipeline_id` field on any `pipeline` record. Absence means "not attributable / not a tracked post-merge regression of a known prior pipeline" — NEVER coerced to a synthetic/guessed link. A missing field is not the same as a confirmed non-regression; aggregations MUST filter (skip) records where the field is absent or falsy rather than treating them as attributed. The post-merge escape rate denominator counts all `record_type == "pipeline"` records; only the numerator requires `triggered_by_pipeline_id` to be present AND truthy (a `null` or empty-string value MUST NOT count — a falsy value is treated identically to an absent key).
...
The auto-learn gate is enforced by the `auto-learn-gate.sh` Stop hook. It fires a visible context message telling the orchestrator to invoke `/harness:learn` when ALL conditions are met:
- ≥3 pipeline observations (`record_type == "pipeline"`) since the last `/harness:learn` run
- ≥3 pipelines since the last `/harness:learn` run, OR no `/harness:learn` has ever run in this project, OR ≥24 hours have elapsed since the last run
- Current pipeline (by `task_id`) is not the one that triggered the most recent firing (idempotency)

Counters are persisted at `~/.claude/learning/{project-hash}/.learn-state.json`. The hook reads only newly-appended observations via a file-offset cursor, so it is idempotent across multiple Stop-event firings within the same turn. The `/harness:learn` skill resets `pipelines_since_learn` and `observations_since_learn` to 0 and stamps `last_learn_run` when it completes (see `skills/learn/SKILL.md` Step 10).
...
Scratchpad findings that recur across 3+ pipelines should be promoted to instincts. The `/harness:learn` skill detects this by comparing scratchpad findings (captured in observations) across pipeline runs:
...
- Same scratchpad finding recurring across 3+ pipelines that ALSO carry `(phases.review.rounds >= 2 OR phases.patch_critic.rounds >= 2)` → emit an **anti-pattern** instinct (`category: anti-pattern`) at confidence `min(0.85, floor + 0.05 * (N - 3))` where `N` is the distinct pipeline count and `floor` is resolved from a domain-weighted map: `{workflow: 0.5, testing: 0.6, code-style: 0.6, architecture: 0.7, security: 0.7}` (default `0.5` for unknown domains). The cap (0.85) is uniform across domains — higher-floor domains reach it with fewer recurrences (architecture/security cap at N=6, testing/code-style at N=8, workflow at N=10). Mining is iron-law-gated on `rounds >= 2` because fix-engineer is dispatched on every CHANGES_REQUESTED AND on every PATCH_REJECTED (per `protocols/pipeline-protocol.md` § In-Cycle Fix Rule), so `rounds >= 2` on either phase is a perfect proxy for "fix-engineer ran". When mining via `phases.patch_critic.rounds`, `/harness:learn` SHOULD tag the resulting anti-pattern instinct's `roles:` with the persona category (`patch-critic-correctness`, `patch-critic-regression`, or `patch-critic-scope`) drawn from `persona_rejections[].persona` so future spawns receive specialty-tuned learning. Legacy observations missing the rounds field on either phase are SKIPPED for that gate clause (the OR still fires if the OTHER clause clears), NEVER coerced to 0. Anti-pattern instincts surface in agent prompts with the `AVOID:` prefix and trigger the +0.1 floor boost described in § Instinct Injection. Anti-patterns are excluded from the Step 7b auto-scaffold scan — they are guidance, not promotable verdicts.

### Instinct Injection (Path-B advisory)

The PreAgent hook `hooks/instinct-injector.sh` (registered on the `Agent` matcher at PreToolUse position 6, between `pre-agent-allowlist.sh` and `depth-guard.sh`) computes which instincts apply to each spawn target and records the resolution to `metrics/{session-id}/instinct-injections.jsonl` for forensic visibility. The hook is **advisory/log-only today** (ADVISORY — NOT ENFORCED — mutation-only): the Agent tool input schema does not yet expose `modified_tool_input`, so the hook cannot patch the spawn prompt. Actual `## Learned Patterns` injection into the prompt body is performed by the orchestrator at spawn time (see `orchestrator/agent-orchestration.md` § Spawn Procedure / § Instinct Injection). When `modified_tool_input` lands, only `hooks/instinct-injector.sh` flips behaviour — resolver, loader, agent frontmatter, and the orchestrator-caller contract are unchanged.

**Why this hook stays advisory while `pre-agent-allowlist.sh` flipped to enforcement on 2026-05-14**: the allowlist gate is **pure-deny** (decision is binary; `exit 2 + stderr` is the canonical delivery), whereas `instinct-injector.sh` is **mutation-semantic** — its value-add is splicing a `## Learned Patterns` block into the prompt, not refusing a spawn. The hook has no shell-side DECISION branch (the resolver is invoked unconditionally, see `hooks/instinct-injector.sh:27`), so there is no place to slot an `exit 2` flip without resolver refactor. More fundamentally, the delivery path is the orchestrator-side splice, not the Agent matcher — the hook is forensic. See `learning/instincts/hook-enforcement-semantics.md` for the pure-deny vs mutation-semantic class boundary.

**Schema-gap evidence (probe RED, 2026-05-14)**: the `promote-advisory-hooks-enforcement` pipeline (2026-05-14) probed whether `modified_tool_input` round-trips on the Agent matcher at v2.1.141. Verdict: RED — the probe could not be safely executed from within a build subagent (settings.json mutation would expose the parent session to cross-hook breakage), and documentary evidence in `protocols/thinking-defaults.md:101-110` confirms the per-spawn schema is still missing. See `$state_dir/promote-advisory-hooks-enforcement/probe-result.md` for the full verdict, environment capture, and re-probe protocol for a future operator-driven session. Until `modified_tool_input` lands AND is verified by manual probe, all mutation-semantic flips (`pre-agent-thinking.sh`, `pre-agent-advisor.sh`, `cache-breakpoint-injector.sh`, this hook) remain advisory.
...
Mismatch (a `logged` record without a paired `orchestrator-injected` record for the same `subagent_type` + `task_id` in the same session) is the Path-B disclosure surface — surfaced post-hoc by `/harness:forensics`.
...
Trigger condition (set by `/harness:learn`): ≥3 pipelines in the same project where a Sonnet executor required ≥2 review rounds for the same role. The flag is data-driven escalation — the system learns when a given (role, project) routinely needs deeper reasoning and starts routing it to Opus automatically.
...
To enable for the current session, invoke `/harness:debug-trace on`. To disable, `/harness:debug-trace off`. The toggle is per-session; the durable default (off) lives in `settings.json` and survives session boundaries. See `skills/debug-trace/SKILL.md` for the full skill contract.
...
### Archive exclusion
- `/harness:learn` (Step 2a) reads `observations.jsonl` only — not `archive/`.
- `/harness:reindex-memory` ingest globs `learning/*/observations.jsonl` — not `archive/`.
...
1. Create scratchpad directory: `mkdir -p $state_dir/{task-id}/scratchpad/`
2. Read session memory: `cat session-memory/{project-hash}/notes.md`
3. Check auto-learn gate: count observations since last `/harness:learn`
...
2. **Check auto-learn gate** — invoke `/harness:learn` if met
