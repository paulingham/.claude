...
2. This enables `/harness:design-qc` to capture screenshots during the pipeline
...
- **Dynamic host port mapping** for the app service (e.g. `ports: ["3000"]`, no host port) so parallel pipeline slices don't collide. `/harness:verify` resolves the mapped port at runtime via `docker compose ... port`.
...
```yaml
# docker-compose.e2e.yml — owned by /harness:verify Tier 4 lifecycle; do not run manually
services:
  app:
    build:
      context: .
    ...
    ports: ["3000"]  # dynamic host port — /harness:verify resolves via `docker compose port`
    depends_on:
...
**Pre-pull on scaffold:** after writing the file, run `docker compose -f docker-compose.e2e.yml pull` once so the first `/harness:verify` invocation isn't dominated by image-pull latency.
