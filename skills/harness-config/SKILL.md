...
**Why this skill exists:** The orchestrator may edit `.md` files in `.claude/` directly (config exception in `rules/agent-protocol.md`). But `.sh` hooks, `.json` settings, and other non-markdown files are NOT covered by that exception. They must be delegated to an agent -- just like source code.
