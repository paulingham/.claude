...
Persist to `$state_dir/{task-id}/product-brief.md`:
```markdown
---
task_id: {task-id}
phase: greenfield
...
Agent({
  ...
  prompt: "Read ~/.claude/knowledge/tech-stack-decision-matrix.md.
    ...
    5. ## Service Topology:
       Unless a forcing function from ~/.claude/protocols/module-boundaries-protocol.md
...
Persist to `$state_dir/{task-id}/tech-stack.md`.
...
Persist to `$state_dir/{task-id}/ui-architecture.md`.
  ...
  prompt: "Bootstrap a new project.
    ...
    3. Create the modular-monolith module directory scaffold (unless the
       ...
       **module**: an in-process unit with a clean public contract and a
       ...
       See `~/.claude/protocols/module-boundaries-protocol.md` for the full
       rules, including:
...
Agent({
  ...
  prompt: "Create placeholder pages for the application.
    UI Architecture: {from $state_dir/{task-id}/ui-architecture.md}
    Design Brief: {from $state_dir/{task-id}/design-brief.md}
...
Update `$state_dir/{task-id}/greenfield.md`:
```markdown
---
task_id: {task-id}
phase: greenfield
