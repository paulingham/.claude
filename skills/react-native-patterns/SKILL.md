...
All React Native code MUST comply with `rules/engineering-protocol.md` (5-line functions, 50-line files, CC <= 5, nesting <= 2, DRY).
...
Maestro YAML flow files are exempt from the 50-line file limit and 5-line function limit defined in `rules/engineering-protocol.md`. E2E flows are sequential step lists, not functions, and splitting them across files harms readability.
