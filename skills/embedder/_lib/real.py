"""Real ctypes+ORT-backed Embedder. Caches api, session handle, vocab."""
...
def build():
    ...
    return RealEmbedder(api, handle, vocab=tokenizer_vocab.load(
        str(tokenizer._DEFAULT_VOCAB)))


class RealEmbedder:
    def __init__(self, api, handle, vocab):
        self._api, self._handle, self._vocab = api, handle, vocab
