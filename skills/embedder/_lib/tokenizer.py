...
def encode(text, max_len=128, vocab_path=None, vocab=None):
    v = vocab if vocab is not None else tokenizer_vocab.load(
        str(vocab_path or _DEFAULT_VOCAB))
    body = tokenizer_encode.words_to_ids(text, v)[: max_len - 2]
    ids = [CLS] + body + [SEP]
    mask = [1] * len(ids)
