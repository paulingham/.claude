...
Mine "what NOT to do" instincts from rework signals. Per `rules/_detail/pipeline-protocol.md` § In-Cycle Fix Rule, fix-engineer is dispatched on every CHANGES_REQUESTED **and** every PATCH_REJECTED, so the gate is the OR clause `phases.review.rounds >= 2 OR phases.patch_critic.rounds >= 2` — either branch alone proves "fix-engineer ran on this pipeline". False positives are worse than misses; legacy observations missing BOTH fields are SKIPPED, never coerced to 0.
