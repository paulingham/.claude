...
_fixture_aggregate() {
  ...
  cat > "$out" <<JSON
{
  "run_id": "r1", "suite": "default", "model": "opus-4-5",
...
check_baseline_writer_frontmatter() {
  ...
  assert "baseline: frontmatter has model"       grep -q "^model: opus-4-5" "$tmp/out.md"
...
check_capture_baseline_writes_file() {
  ...
  local expected="$tmp/eval/baselines/2026-04-24-opus-4-5.md"
  assert "capture: committed-path baseline exists" is_file "$expected"
  assert "capture: latest symlink exists"         _islink "$tmp/eval/baselines/latest-opus-4-5.md"
  rm -rf "$tmp"
}
...
check_capture_baseline_symlink_updates() {
  ...
  local link="$tmp/eval/baselines/latest-opus-4-5.md"
  local target; target="$(readlink "$link")"
  assert "capture: symlink updated to newest"  _eq "$target" "2026-04-24-opus-4-5.md"
  rm -rf "$tmp"
}
