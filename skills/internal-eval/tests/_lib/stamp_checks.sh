...
## Per-Case Results
MD
  (cd "$dir" && rm -f "latest-opus-4-7.md" && ln -s "$file" "latest-opus-4-7.md")
}
...
check_stamp_emits_section_with_baseline() {
  local score="$1"; local tmp; tmp="$(mktemp -d)"
  _stamp_fixture_baseline "$tmp/baselines" "2026-04-24-opus-4-7.md"
...
check_stamp_has_harness_ref_timestamp_date() {
  ...
  assert "stamp: contains model"          grep -q "Model.*opus-4-7" <<<"$out"
...
check_stamp_baseline_file_link() {
  ...
  assert "stamp: includes baseline file path" \
    grep -q "eval/baselines/2026-04-24-opus-4-7.md" <<<"$out"
  rm -rf "$tmp"
}
...
check_stamp_missing_field_graceful() {
  ...
  cat > "$tmp/baselines/2026-04-24-opus-4-7.md" <<'MD'
---
baseline_date: 2026-04-24
model: opus-4-7
harness_ref: abc1234
...
---
MD
  (cd "$tmp/baselines" && ln -s "2026-04-24-opus-4-7.md" "latest-opus-4-7.md")
