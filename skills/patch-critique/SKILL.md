...
Verdict gates Ship. PATCH_REJECTED returns to fix-engineer per `rules/pipeline-protocol.md` § In-Cycle Fix Rule — never escalates to the user.
...
This skill belongs to the `final-gate` parallel group. Dispatched via Parallel Dispatch Protocol (`rules/parallel-dispatch-protocol.md` § Final Gate Team), not via sequential Skill tool invocation.
...
PATCH_REJECTED is gated by `rules/pipeline-protocol.md` § In-Cycle Fix Rule:
