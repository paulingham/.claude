...
# (use code-shape-check logic: per-language function limits — see protocols/engineering-invariants.md § Code Shape)
...
Write to `$state_dir/health-reports/{date}.md` (relocated under a `health-reports/` subdir — health is a project-wide concern, not task-scoped; the legacy flat path `$state_dir/health-report-{date}.md` is deprecated to remove namespace overlap with task-id `health-report-2026-04-30` per R12):
...
Report: $state_dir/health-reports/{date}.md
