...
3. **`agents/session-memory-updater.md` has no primary skill.** Same shape as fix-engineer: dispatched with a hand-built spawn prompt (notes path + curated facts) by the orchestrator. The procedure lives entirely in the agent file plus `protocols/autonomous-intelligence.md` § Update Mechanism.
