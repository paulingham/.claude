...
How the harness shares engineering state (session memory, learning loop, manifests, memory.sqlite) across session worktrees of `$HOME/.claude` while keeping per-branch state isolated. Cross-referenced from `protocols/agent-protocol.md` and `protocols/autonomous-intelligence.md`.
