...
- [ ] **Add the new hook to the inventory set in `tests/test_stop_hook_active_short_circuit.py`.** That regression test pipes `{"stop_hook_active": true}` into every registered Stop / SubagentStop hook and asserts exit 0 with no side effects (no metrics dir, no pipeline-state writes, no stdout). It also statically checks that the EXIT trap precedes the first early-exit. The inventory assertion fails closed: a new hook lands without the guard → CI fails.
...
| `arch-fitness-gate.sh` | SubagentStop | yes | duplicate arch-fitness JSONL row |
