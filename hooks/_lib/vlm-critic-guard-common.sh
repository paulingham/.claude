#!/usr/bin/env bash
# Common parsing + violation-logging helpers shared by the vlm-critic
# guard(s). This file is a VERBATIM CLONE of
# hooks/_lib/spec-blind-guard-common.sh with the public-function prefix
# renamed from `_spec_blind_*` to `_vlm_critic_*`. The clone-not-extract
# decision is forced by the spec-blind V2 soak (ends 2026-06-09).
#
# Consolidation target: `hooks/_lib/tool-isolation-guard-common.sh`. The
# follow-up extraction pipeline is anchored at
# `pipeline-state/vlm-spec-blind-common-extract-soak-end/pipeline.md`
# (not_before 2026-06-09T00:00:00Z). At that point this file and
# `hooks/_lib/spec-blind-guard-common.sh` collapse into thin shims over the
# generic library.
#
# Public functions:
#   _vlm_critic_parse_input  — sets TOOL_NAME / SUBAGENT_TYPE / SESSION /
...
#   _vlm_critic_redact <str> — applies the canonical secret-redaction patterns
...
_vlm_critic_log_violation() {
  ...
  if [[ -n "$offender" ]]; then
    jq -nc \
      ...
      '{ts:$ts, record_type:"vlm_critic_blocked", subagent_type:$subagent, tool:$tool, ($field):$target, offender:$offender, session_id:$session, guard:$guard, action:"blocked"}' \
      >> "$log_file" 2>/dev/null
  else
    jq -nc \
      --arg ts "$ts" \
      ...
      --arg guard "$guard" \
      --arg field "$target_field" \
      '{ts:$ts, record_type:"vlm_critic_blocked", subagent_type:$subagent, tool:$tool, ($field):$target, session_id:$session, guard:$guard, action:"blocked"}' \
      >> "$log_file" 2>/dev/null
  fi
}
