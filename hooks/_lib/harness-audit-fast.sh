...
_haf_count_orphan_hooks() {
  local hooks_dir="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hooks" count=0
  while IFS= read -r f; do
    grep -qF "$(basename "$f")" "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/settings.json" 2>/dev/null || count=$((count+1))
  done < <(find "$hooks_dir" -maxdepth 1 -name "*.sh" 2>/dev/null)
  echo "$count"
}

_haf_check_agents_frontmatter() {
  local agents_dir="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/agents" count=0
  while IFS= read -r f; do
