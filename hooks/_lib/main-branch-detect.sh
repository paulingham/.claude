#!/usr/bin/env bash
# Main-branch invariant detection. Bash 3.2 SAFE; ERE only. Per-clause check
# is the unit; `cd <path> &&` is the only whole-command early-exit because cd
# persists across `&&`. `git -C` / `--git-dir=` are self-contained per clause.
...
is_forbidden_clause() {
  ...
  _mbd_is_safe_fetch "$norm" && return 1 || return 0
}
