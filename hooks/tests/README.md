...
| test-managed-settings.sh | — | QUARANTINED — plugin-delivery drift only (A2/A4 delivery keys may still drift on fresh install); env-keys drift (A3 now requires 8 keys; CLAUDE_PIPELINE_MODE + CLAUDE_ENABLE_TRACE moved to user layer) resolved by slice-toggle-layering |
