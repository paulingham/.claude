...
# BUG_FIXED payload-shape enforcement only has meaning for Build/Pipeline
# gear work. sid comes from this SubagentStop hook's own stdin JSON.
check_gear_gate "$(resolve_session_id "$INPUT")" || { trap - EXIT; exit 0; }
