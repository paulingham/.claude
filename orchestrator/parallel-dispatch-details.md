...
5. **Spawn ONE code-reviewer teammate for selection** with the rubric:
   ...
   - `diff_size`: tie-breaker term — captured per-candidate in Step 4 via `git diff --stat main..<branch>` (records both `changed_files` and `changed_lines`). Spec for tie-break ordering: `(changed_files, changed_lines)` ascending — fewer files first, fewer lines second. `score.sh::pick_winner` consumes a 5-field record `name|score|changed_files|changed_lines|cost_rank`; sort ordering is score DESC primary, then `(changed_files_asc, changed_lines_asc, cost_asc)` ASC. `skills/best-of-n/config.json::tie_breaker_order` records `["changed_files_asc", "changed_lines_asc", "cost_asc"]` to match the script's sort-key tuple.
...
SendMessage({
  to: "code-reviewer",
  message: "Re-review: Finding F1 (method body exceeds per-language limit) was addressed.
