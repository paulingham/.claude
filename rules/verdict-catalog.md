...
| `SANDBOX_SKIPPED` | info | `sandbox-verify` | build | Sandbox unavailable; reason ∈ {`no-e2b-token`}; one JSONL line appended to `metrics/{session-id}/sandbox-verify-skips.jsonl`; Build advances |
