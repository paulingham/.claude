"""Python shim that invokes the bats suite for scripts/new-session.sh.

Real assertions live in tests/shell/new-session.bats. This wrapper integrates
...
BATS_SPEC = REPO_ROOT / "tests" / "shell" / "new-session.bats"


class NewSessionBatsSuite(unittest.TestCase):
...
class NewSessionStaticChecks(unittest.TestCase):
    """Static checks for new-session.sh and its _lib helpers."""

    SCRIPTS_DIR = REPO_ROOT / "scripts"
    TARGETS = (
        "new-session.sh",
        "_lib/session-paths.sh",
        "_lib/session-name.sh",
    )
    ...
    def test_shape_line_budgets(self) -> None:
        budgets = {
            "new-session.sh": 50,
            "_lib/session-paths.sh": 25,
            "_lib/session-name.sh": 20,
        }
