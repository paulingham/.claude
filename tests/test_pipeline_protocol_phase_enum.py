...
def test_plan_in_pipeline_protocol_phase_enum():
    content = (Path(__file__).parent.parent / "protocols" / "pipeline-protocol.md").read_text()
    # The phase enum line should contain 'plan'
    assert re.search(r'phase.*plan.*build', content), (
        "protocols/pipeline-protocol.md phase enum must include 'plan' before 'build'"
    )


def test_planning_agent_subsection_in_parallel_dispatch():
    content = (Path(__file__).parent.parent / "protocols" / "parallel-dispatch-protocol.md").read_text()
    assert "Planning Agent" in content, (
        "protocols/parallel-dispatch-protocol.md must have a Planning Agent subsection"
    )
