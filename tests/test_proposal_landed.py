...
class ProposalRound2Refinements(unittest.TestCase):
    ...
    def test_proposal_contains_operator_copy_table_8_rows(self):
        text = PROPOSAL.read_text()
        ...
        # 8 data rows: fresh, state_file_missing, git_head_mismatch, hard_staleness,
        # no_worktree_resolvable, sandbox_staleness, state_file_parse_error, git_timeout.
        self.assertEqual(len(rows), 8,
                         f"Operator Copy table must have 8 data rows, found {len(rows)}")
