#!/usr/bin/env bash
# BUG_FIXED payload without red_evidence in strict mode → exit 2 + helpful stderr.
set -uo pipefail
...
NAME="missing_red_strict_rejects"
if [[ "$RC" -eq 2 ]] && echo "$OUT" | grep -q "missing required key: red_evidence"; then
  echo "PASS: $NAME"
  exit 0
else
